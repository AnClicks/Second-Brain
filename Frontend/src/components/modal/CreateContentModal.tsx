...
export function CreateContentModel({ open, onClose }:any) {
    ...
    return <div >
        {open && <div className='backdrop-blur-xs w-screen h-screen bg-black/60 fixed top-0 left-0 flex justify-center' ref={modalRef} onClick={(event)=>{ if(event.target == modalRef.current){
            onClose()
        }}}>
            <div className="flex justify-center flex-col">
                <span className="bg-white p-4 rounded-xl">
                    ...
                    <div className="flex flex-col">
                        <Input color='blue-200'ref={titleRef} placeholder={"Enter Title"}></Input>
                        <Input color='blue-200' ref={linkRef} placeholder={"Enter Link"}></Input>
                    </div>
